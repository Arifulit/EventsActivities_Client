...
async function fetchBackendHostDetails(hostId: string, request: NextRequest) {
	if (!API_BASE || !API_BASE.startsWith('http://') && !API_BASE.startsWith('https://')) {
		return null;
	}

	// Use the /users/:userId/details endpoint
	...
	try {
		...
		console.log('Backend response received:', data?.success);
		return data;
	} catch (error) {
...
export async function GET(
	...
	console.log('API Route - Host ID:', hostId);
	...
	const backendResponse = await fetchBackendHostDetails(hostId, request);
